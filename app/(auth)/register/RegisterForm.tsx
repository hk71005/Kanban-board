  ...
  const onSubmit = (values: z.infer<typeof registerSchema>) => {
    ...
    startTransition(async () => {
      ...
      if (data.success) {
        ...
        // Hand off to the invite page — server-side acceptance happens there
        if (inviteData?.token) {
          router.push(`/invite/${inviteData.token}`);
          return;
        }

        // No invite — go to personal board
        const dest = data.boardId ? `/boards/${data.boardId}` : '/boards';
        router.push(dest);
      }
    });
  };
    ...
    >
      <Card className="w-full max-w-md mx-4">
        <CardHeader className="text-center">
          ...
          {inviteData ? (
            <>
              <CardTitle className="text-2xl font-bold">Join {inviteData.boardTitle}</CardTitle>
              <CardDescription>
                {inviteData.inviterName} invited you. Create your free account to join.
              </CardDescription>
            </>
          ) : (
            <>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        type="email"
                        disabled={isPending || !!inviteData}
                        className={inviteData ? 'opacity-75 cursor-not-allowed' : ''}
                      />
                    </FormControl>
                    {inviteData && (
                      <p className="text-xs text-muted-foreground">
                        Your account must use this email to accept the invite.
                      </p>
                    )}
                    <FormMessage />
                  </FormItem>
              ...
              <Button type="submit" className="w-full" disabled={isPending}>
                ...
                {inviteData ? 'Create account and join' : 'Register'}
              </Button>
            </form>
          </Form>
